%include {
		...
		FieldMethodDefList* fmList = new FieldMethodDefList;
		...
		void addToList(VariableDefList* source, FieldMethodDefList* target){
				VariableDefList::iterator it = source->begin();
				while (it != source->end()) {
					VariableDef *var = *it;
...
%type op_field_and_methods_list {FieldMethodDefList*}
...
class_def(A) ::= K_CLASS ID(B) T_OSET op_field_and_methods_list T_CSET. { A = new ClassDef(B->strValue, fmList); }

op_field_and_methods_list::= field_and_methods_list.
op_field_and_methods_list::= .

field_and_methods_list::= field_and_methods_list field_and_methods.
field_and_methods_list::= field_and_methods.

field_and_methods::= field_decl(B).		{ addToList(B, fmList); }
field_and_methods::= method_decl(B).	{ fmList->push_back(B); }
...
method_decl(A)::= type(B) ID(C) T_OPAR opt_parameter_decl_list(E) T_CPAR block(G).
																																		{
